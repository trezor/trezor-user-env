...
If you are using NixOS you can do the changes locally and then run the controller yourself. Run it via `nix-shell controller/shell.nix --run 'python controller/main.py'`. Make sure you have run `{firmware,trezord-go}/bin/download.sh` beforehand otherwise you'll have old binaries.
