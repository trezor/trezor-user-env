...
If you are using NixOS you can do the changes locally and then run the controller yourself. Run it via `nix-shell --run 'uv run python src/main.py'`. Make sure you have run `uv run python src/binaries/firmware/bin/download.py` and `src/binaries/trezord-go/bin/download.sh` beforehand otherwise you'll have old binaries.
